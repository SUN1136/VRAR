...
public class GenerateObject : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        if (Physics.Raycast(rayObject, out hit, mask))
        {
            Debug.Log("I hitted the ::::::::::::::::::: " + hit.collider.gameObject.name);
            if (inputLinker.rightGrab && Grabbed == false)
            {
                GameObject obj = null;
        ...
        // If we left the rightGrab, Then all the function works again
        if (!inputLinker.rightGrab && Grabbed == true)
        {
            objectToDrag = null;
            Grabbed = false;
            rotAngle = 0;
